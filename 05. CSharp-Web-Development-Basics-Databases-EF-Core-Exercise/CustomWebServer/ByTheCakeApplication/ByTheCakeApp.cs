namespace CustomWebServer.ByTheCakeApplication
{
    using Controllers;
    using Data;
    ...
    public class ByTheCakeApp : IApplication
    {
        ...
        {
            using (var dbContext = new ByTheCakeDbContext())
            {
        ...
        public void Configure(IAppRouteConfig appRouteConfig)
        {
            ...
            appRouteConfig
                .Post("/add", req => new ProductsController().Add(
            ...
            appRouteConfig
                .Get("/productDetails/{(?<id>[0-9]+)}", req => new ProductsController()
            ...
            appRouteConfig
                .Post("/register", req => new AccountController()
            ...
            appRouteConfig
                .Post(
                    "/login", req => new AccountController()
                    .Login( req, new LoginViewModel
                        {
                            Username = req.FormData["username"],
                            Password = req.FormData["password"]
                        }));
            ...
            appRouteConfig
                .Get("/orders", req => new ShoppingController().UserOrders(req));

            appRouteConfig.
               Get("/orderDetails/{(?<id>[0-9]+)}", req => new ShoppingController().OrderDetails(req));

            appRouteConfig
                .Get("/orderDetails/productDetails/{(?<id>[0-9]+)}", req => new ProductsController()
                        .Details(int.Parse(req.UrlParameters["id"])));
        }
    }
}
